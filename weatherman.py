...
def yearly_summary(year, data_folder, city):
    ...
    data = [row for row in data if len(row) > 5 and row[2] and row[3] and row[5] and row[2].replace('.', '', 1).isdigit() and row[3].replace('.', '', 1).isdigit() and row[5].replace('.', '', 1).isdigit()]
...
def monthly_summary(year, month, data_folder, city):
    ...
    data = [row for row in data if len(row) > 6 and row[2].replace('.', '', 1).isdigit() and row[3].replace('.', '', 1).isdigit() and row[6].replace('.', '', 1).isdigit()]
...
def draw_horizontal_bar_charts(year, month, data_folder, city):
    ...
    for row in data:
        if row[2] and row[3] and row[2].replace('.', '', 1).isdigit() and row[3].replace('.', '', 1).isdigit():  # Ensure there are values for max and min temperatures
            date = row[0]
